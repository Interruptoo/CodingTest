...
namespace Programers_CodingTest
{
     ...
     */
    internal class 혼자놀기의달인
    {
        private static void Main(string[] args)
        {
            Solution s = new Solution();

            Console.WriteLine(s.solution([8, 6, 3, 7, 2, 5, 1, 4]));
    ...
    class Solution
    {
        ...
        {
            int answer = 0;
            List<List<int>> totalBox = new List<List<int>>();
            var usedArray = cards.ToList();

            while (usedArray.Count > 0)
            {
                int cursor = usedArray.First();
                List<int> currentBox = new List<int>();

                while (!currentBox.Contains(cards[cursor - 1]))
                {
                    currentBox.Add(cards[cursor - 1]);
                    usedArray.Remove(cursor);
            ...
            if(totalBox.Count > 1)
            {
                ...
                foreach (var item in totalBox.OrderByDescending(o => o.Count).Take(2).ToList())
                {
                    answer *= item.Count;
                };
            }
